...
\pagenumbering{roman}                   % číslování stránek římskými číslicemi
	...
	\abstrakt{
					Tato práce se zabývá navržením a implementací vědomostních her. V~první části práce se čtenář seznámí s~pojmem vědomostní hra a s~několika příklady takových her na současném trhu. Dále se seznámí s~moderním postupem při vývoji aplikací, s~tzv. agilními metodami. Druhá část práce se zabývá návrhem a programováním nové vědomostní hry za využití těchto agilních metod.			% Abstrakt 
	}{
					Vědomostní hra, Agilní metody, Extrémní programování, Scrum, Python, Pygame			% Klíčová slova
	}
	
	\podekovani{
			Děkuji především vedoucímu této maturitní práce, Dr.rer.nat. Michalu Kočerovi, za trpělivost a za zodpovídání mých dotazů.	 Dále děkuji své rodině a svým přátelům za podporu při psaní této práce.		% Poděkování
	}
...
\chapter*{Úvod}     % úvod práce 
	...
	První část práce se zaměří na současný stav vědomostních her. Konkrétně se bude zabývat hrami \textit{Dobyvatel}, \textit{Sporcle} a \textit{Kahoot!}, jejich historií i současnou podobou.
	
	Dále se práce bude zabývat agilními metodami. Přiblíží čtenáři pojem \textit{Agilní vývoj softwaru} a představí mu některé takové metody a jejich přínosy při vývoji softwaru. Těmito metodami budou \textit{Extrémní programování}, \textit{Testováním řízený vývoj} a \textit{Scrum}.
	
	V~hlavní části práce se pokusím vyvinout svou vlastní vědomostní hru. Budu se přitom snažit co nejvíce řídit zásadami agilního vývoje. Pro psaní kódu využiji programovací jazyk \texttt{Python} a jeho rozšíření \texttt{Pygame}. V~práci poté popíši postup vývoje této hry a její konečnou podobu.
...
Odpovědí na \textit{tipovací otázku} je vždy celé nezáporné číslo a vyhrává ten hráč, kterého odpověď byla nejblíže té správné. Pokud je několik odpovědí stejně daleko, vyhrává ten hráč, který odpověděl jako první. U~\textit{otázek s~výběrem správné odpovědi} mají hráči vždy na výběr ze čtyř možných odpovědí, z~nichž je vždy právě jedna správná \cite{Dobyvatel2018}.
    ...
    \begin{minipage}{.5\textwidth}
        ...
        \caption{Ukázka GUI „starého“ \textit{Dobyvatele} \cite{2013}}
        \label{dobyvatel_fig}
    \end{minipage}%
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[height=130pt]{Novy_dobyvatel}
        \caption{Ukázka GUI „nového“ \textit{Dobyvatele} \cite{Meciar2025a}}
        \label{dobyvatel_fig}
    \end{minipage}
    \end{figure}
...
\begin{figure}[h]
    \begin{minipage}{.5\textwidth}
        ...
        \caption{Ukázka GUI mobilní aplikace \textit{Dobyvatel} \cite{Dobyvatel2025}}
        \label{dobyvatelGUI}
    \end{minipage}
    \begin{minipage}{.5\textwidth}
        \centering
...
\end{figure}
   ...
   Už v~roce 2013 \textit{Sporcle} přesáhlo jednu miliardu zahraných kvízů a v~roce 2014 přesáhlo 500~000 vytvořených kvízů. V~současnosti \textit{Sporcle} dosahuje asi milion zahraných kvízů denně a~téměř šest a půl miliardy zahrání celkem \cite{Sporcle2025a}. Nejhranější kvíz, s~osmdesáti miliony přehrání, se jmenuje „Find the US States - No Outlines Minefield“,\cite{Sporcle2026} v~překladu „Najdi státy USA – bez obrysů, minové pole“. Minové pole v~názvu znamená, že pokud hráč udělá chybu, celý kvíz se ukončí \cite{Sporcle2026a}.

\section{Kahoot!}
\textit{Kahoot!} je Norská online vzdělávací platforma umožňující lidem tvořit a hrát kvízy založené na otázkách s~výběrem správné odpovědi \cite{Kahoot2025}. Původní prototyp této platformy byl vytvořen už v~roce 2006, ovšem k~největšímu rozmachu došlo až v~roce 2020 kvůli pandemii COVID-19 \cite{wang2007lecture}\cite{Hanoa2020}.\\
...
Morten Versvik, Johan Brand, and Jamie Brooker založili společnost \textit{Kahoot!} v~roce 2012. Technologie této společnosti je zložena na výzkumu, jenž provedl Morten Versvik, jehož učil Alf Inge Wang, který se také zapojil do vývoje této vědomostní hry. V~roce 2013 byla tato platforma poprvé zpřístupněna pro veřejnost \cite{Kahoot2025}. Už v~roce 2017 měla tato hra přes 40~milionů aktivních hráčů měsíčně a tento rok také společnost \textit{Kahoot!} vydala mobilní aplikaci, skrze kterou mohou učitelé zadávat žákům domácí ůkoly  \cite{Keane2017}\cite{Ravipati2017}.

\subsection{Průběh hry}
Jednotlivé kvízy se nejčastěji hrají pomocí jedné velké obrazovky a mobilních telefonů pro každého hráče. Na velké obrazovce se zobrazí otázka a výběr odpovědí a hráči pomocí mobilních zařízení odpovídají. Každý hráč hraje pod zvolenou přezdívkou, takže kvíz může být anonymní, což pomáhá snižovat strach ze špatné odpovědi. Za každou správnou odpověď dostanou hráči počet bodů podle toho, jak rychle odpověděli. Mezi otázkami se vždy zobrazí na velké obrazovce správná odpověď a distribuce odpovědí, což dává zpětnou vazbu tomu, kdo kvíz vytvořil. Například, pokud většina hráčů zadala stejnou špatnou odpověď, mohla být otázka položena nepřesně. Poté se na obrazovce zobrazí přezdívky a body pěti nejúspěšnějších hráčů. Zároveň každý hráč vidí individuální zpětnou vazbu na své obrazovce. Hráč s~největším počtem bodů vyhrává. Tento styl hry se dá využívat například ve školách, ve firmách nebo na různých společenských akcích \cite{Wang2015}.\\

Druhá možnost jak hrát \textit{Kahoot!} nepotřebuje velkou obrazovku. Každý hráč na svém zařízení vidí otázku i možné odpovědi a kvíz si může zahrát kdekoli a kdykoli. Tuto možnost je vhodné používat například na domácí úkoly, školení, atp. \cite{Kahoot2025a}.
...
\chapter{Agilní metody vývoje softwaru}
	...
	Agilní metody vývoje softwaru jsou takové metody, které následují hodnoty \textit{Manifestu Agilního vývoje software} a jeho 12 principů. Hlavními body \textit{Agilního manifestu} jsou: 
	\begin{itemize}
...
\item Reagování na změny před dodržováním plánu \cite{Beck2001}
	\end{itemize}
	
První bod poukazuje na jeden z~nejdůležitějších rozdílů mezi agilním a jinými přístupy k~vývoji softwaru, kterým je zaměření na lidi, jejich interakce mezi sebou a jejich spolupráci. Druhý bod manifestu říká, že i když je dokumentace důležitá, hlavní prioritou vývojářů by mělo být dodávat funkční software. Při agilním vývoji softwaru je nutná neustálá komunikace se zákazníkem a tuto skutečnost odráží třetí bod manifestu. Tato spolupráce se zákazníkem může často vést k~změnám v~požadavcích na vyvíjený software. Změna je pro agilní vývoj softwaru velice důležitá, jak dokazuje poslední bod manifestu. Vývojáři pracující agilně tedy vítají změnu požadavků i během samotného vývoje. I~proto zvolili autoři název \textit{Agilní}, protože odráží důležitost přizpůsobivosti a připravenosti na změnu \cite{AgileAlliance2025}.\\

Dvanáct principů agilního vývoje softwaru dávají jasnější pokyny k~zavedení agilního postupu. Tyto principy vyzdvihují individualitu, neustálou spolupráci se zákazníkem, samoorganizující se týmy, osobní konverzaci, často dodávaný jednoduchý software a důležitost změny \cite{Beck2001a}.

		\section{Historie}
		
		Za počátek agilního vývoje se většinou považuje schůze 17 vývojářů v~Utahu v~roce 2001. Agilní přístup ovšem existoval i předtím, jen nebyl ucelený a neměl zatím název. Už v~polovině devadesátých let vymýšleli lidé nové postupy, které se podobaly dnešnímu agilnímu vývoji. \textit{Manifest Agilního vývoje software} vznikl právě na již zmíněné schůzi v~Utahu. Tento manifest shrnul základní myšlenku a poprvé zde byl užit termín \textit{Agilní vývoj softwaru}. V~následujících měsících doplnili autoři manifest o~12 principů agilního vývoje softwaru. Na konci roku 2001 byla také vytvořena\textit{ Agile Alliance}. Jejím cílem je spojovat lidi vyvíjející software, aby mohli sdílet nápady a zkušenosti \cite{AgileAlliance2025a}.
		
		\section{Extrémní programování}
		
		Jednou z~agilních metod vývoje je tzv. Extrémní programování. Jedná se o~metodiku pro malé až střední týmy, které vyvíjejí software a mají nejasné, nebo často se měnící zadání. Cíl extrémního programování je snižovat projektová rizika, zlepšit schopnost reagovat na změny a zvýšit produktivitu \cite{Beck2002}. \\
  ...
  \item Nepřetržitá integrace (Nový kód by se měl několikrát denně integrovat do stávajícího), 
  ...
  \item Standarty pro psaní zdrojového kódu (Zdrojový kód by měl mít jednotnou strukturu) \cite{Beck2002}
		\end{itemize}
		
		Jedním z~kontroverznějších z~těchto postupů je bezpochyby párové programování. Jedná se o~postup, při kterém programují dva lidé zároveň na jednom počítači, s~jednou klávesnicí a~jednou myší. Celý proces spočívá v~tom, že vždy jeden z~programátorů píše a druhý ho kontroluje a zároveň přemýšlí nad dalším problémem, dalším testem nebo možnou refaktorizací. Výhodou oproti tradičnímu postupu je například fakt, že při párovém programování se více chyb odhalí už při samotném psaní kódu. Další výhodou je i to, že když člověk programuje s~někým, tak bude nejspíše produktivnější, protože se z~úcty ke druhému nenechá rozptýlit telefonátem nebo jinou soukromou záležitostí. To nás ale přivádí k~jedné z~nevýhod a tou je to, že tento proces funguje pouze za předpokladu, že se tito dva lidé vzájemně snesou a~mají k~sobě dostatek respektu \cite{Beck2002}.\\
		
		Podle metaanalýzy z~roku 2009 nelze jednoznačně určit, zda je párové programování lepší než sólo programování, ale vždy záleží na dané situaci. Párové programování je totiž užitečnější, pokud potřebujeme jednoduchý úkol splnit co nejrychleji, i za cenu ztracené kvality, nebo potřebujeme splnit složitý úkol kvalitněji, i za vyšší cenu \cite{Hannay2009}. Párové programování také samozřejmě nebude lepší, pokud se členové týmu nerespektují, nebo se nesnesou. V~takovém případě je nutné, aby spolu tito lidé nepracovali a někdy toto může vést i k~tomu, že takový člověk musí z~týmu odejít \cite{Beck2002}.\\
		
		I~přesto, že párové programování nemusí být vždy lepší, mají vývojáři na extrémní programování pozitivní názory. Většina takto pracujících vývojářů by u~tohoto postupu chtěla zůstat. Tito vývojáři mají také vyšší uspokojení z~práce a věří, že jsou produktivnější. Podobné názory převládají také mezi studenty. Většina z~dotázaných věřila, že extrémní programování zlepšuje kvalitu kódu a tento postup by doporučila svým budoucím zaměstnavatelům \cite{Dybaa2008}.\\
		
		Zda extrémní programování opravdu zvyšuje produktivitu vývojářského týmu se ovšem nedá jednoznačně určit. Podle jedné studie se produktivita, měřena počtem napsaných řádků kódu, zvedla o~46 \%, ale postupně s~časem začala klesat zpět na stejné hodnoty, jaké měl tým využívající tradiční metody \cite{Ilieva2004}. Oproti tomu jiná studie zjistila 44\% pokles v~produktivitě při používání extrémního programování \cite{Dybaa2008}.\\

\section{Testováním řízený vývoj}
\textit{Testováním řízený vývoj} je metoda vývoje softwaru, která se snaží o~\textit{čistý kód, který funguje}. Tohoto se snaží dosáhnout dvěma pravidly: Pište nový kód, pouze pokud selhal automatický test a Eliminujte duplicitu. Tyto dvě pravidla určují postup při psaní nového kódu:
\begin{enumerate}
  \item Napište malý test, který ze začátku nefunguje
  \item Rychle zařiďte, aby test fungoval i za cenu špatného kódu
  \item Refaktorizujte—odeberte duplicitu způsobenou předchozím krokem
\end{enumerate}
Důvodem pro psaní testů je zamezení strachu. Kvůli strachu totiž můžeme být nerozhodní, málo komunikovat a bát se zpětné vazby. Právě automatizované testy nám ale mohou pomoci se méně bát. Víme totiž, že pokud všechny testy fungují, pak i celý kód bude fungovat a~pokud nějaký fungovat přestane, hned víme, kde máme hledat chybu. Další způsob, jak tyto testy eliminují strach je to, že nám pomohou si větší úkol rozdělit do mnoha malých \cite{Beck2015}.\\

Podle systematického přehledu z~roku 2010, který zkoumal 32 různých studií se zaměřením na efektivitu \textit{Testováním řízeného vývoje}, se nedá jednoznačně určit, zda je tato metoda vývoje softwaru lepší, než klasické postupy. Celková kvalita softwaru ani produktivita nebyla jednoznačně lepší, nebo horší než při využití jiných postupů. Jediná oblast, ve které byla tato metoda lepší, je kvalita testů, ale ani ta nebyla o~tolik vyšší, než kolik se předpokládalo. Ve výsledku tedy opět záleží na jednotlivém týmu, zda bude tento postup efektivnější, či nikoli \cite{Turhan2010}.

\section{Scrum}

\textit{Scrum} je odlehčený rámec založený na krátkých časových intervalech, tzv. \textit{Sprintech} a na \textit{Scrum týmu}. \textit{Scrum tým} se skládá z~vývojářů, \textit{Vlastníka produktu} a z~\textit{Mistra Scrumu}. \textit{Vlastník produktu} se snaží maximalizovat hodnotu výsledného produktu. \textit{Mistr Scrumu} dohlíží na dodržování hodnot a postupů v~rámci\textit{ Scrum}. Pomáhá také každému členu týmu \textit{Scrumu} porozumět \cite{Schwaber2020}.\\

 \textit{Sprint} je označení pro vývojový cyklus \textit{Scrum týmu}. Jeden \textit{Sprint} by měl trvat maximálně jeden měsíc, ale mohou trvat i kratší dobu. Na začátku každého \textit{Sprintu} by mělo proběhnout \textit{Plánování Sprintu}. Během tohoto plánování \textit{Vlastník produktu} navrhuje, jak v~tomto \textit{Sprintu} zvýšit hodnotu produktu a společně s~celým týmem sestavuje \textit{Cíl Sprintu}. Vývojáři vyberou úkoly, které budou zahrnuty do \textit{Sprintu}. Celé plánování by nemělo trvat déle než osm hodin pro \textit{Sprint} o~délce jednoho měsíce. Další součástí \textit{Sprintu} je \textit{Denní Scrum}, což je patnáctiminutová schůze \textit{Scrum týmu}, konaná každý den ve stejný čas a na stejném místě. Na této schůzi vývojáři vytvoří plán pro daný den. \textit{Sprint} končí \textit{Revizí Sprintu}, jejímž úkolem je prezentace výsledků daného \textit{Sprintu} a diskuze mezi \textit{Scrum týmem} a důležitými zúčastněnými stranami. Tato revize by neměla trvat déle než čtyři hodiny pro \textit{Sprint} o~délce jednoho měsíce \cite{Schwaber2020}.\\
 
Podle studie z~roku 2005 jsou názory na zavedení \textit{Scrumu} pozitivní mezi vývojáři i mezi zákazníky. Všichni dotázaní vývojáři by tento rámec doporučili i v~budoucnu. Po zavedení \textit{Scrumu} byly vývojáři spokojenější s~odvedenou prací a byli si jistější, že vytvářejí produkt, se kterým bude spokojen i zákazník. Zákazníci také byly spokojenější s~výsledným produktem a~také by v~budoucnu \textit{Scrum} doporučili. Zákazníci začali být také více zaujatí prací vývojářů, protože se stali součástí vývoje, a také k~nim začali mít větší respekt. Mimoto se se zavedením \textit{Scrumu} snížil počet odpracovaných přesčasů, což vede ke udržitelnějšímu vývoji \cite{Mann2005}.
...
\chapter{První verze}
	...
	Začala jsem importováním modulu \texttt{csv}, který jsem použila pro manipulaci s~databází otázek, která měla formu CSV souboru. Poté jsem nadefinovala třídu \texttt{Otazka}. Tato třída má čtyři atributy, a těmi jsou: \texttt{Typ}, \texttt{jenOtazka}, \texttt{spravnaOdpoved}, \texttt{vsechnyOdpovedi}. Její konstruktor má parametr \texttt{cisloOtazky}. Tento konstruktor vyhledá v~tabulce otázku s~požado-vaným číslem a přiřadí atributům třídy odpovídající proměnné.   
...
Další metodou třídy \texttt{Otazka} je \texttt{tah}. Má jeden parametr \texttt{hrac} (objekt třídy \texttt{Hrac}). Tato metoda vybere náhodnou otázku pomocí funkce \texttt{randint} modulu \texttt{random}. Poté zavolá metodu \texttt{zeptej} a pokud vrátí \texttt{True}, připíše hráči jeden bod.
...
Poslední metodou třídy \texttt{Demo} je \texttt{run}. Tato metoda postupně zavolá funkci \texttt{tah} pro každého hráče a poté vytiskne aktuální skóre na terminál. Toto opakuje několikrát, podle toho, jaký počet kol byl zadán.\\
...
\section{Verze 1.1}
Tato verze se od té první liší tím, že místo CSV souboru využívá SQL databázi. Pro práci s~touto databází využívá modul \texttt{Databaze}. Tento modul má jednu třídu s~názvem \texttt{DbOtazek}. V~konstruktoru této třídy se vytvoří tabulka s~názvem otazky a strukturou zobrazenou v~tabulce 4.1.\

\begin{table}[h]
\begin{center}
 		\caption{Struktura databáze otázek}
\begin{tabular}{ |p{1cm}|p{3cm}|p{1cm}|  }
 \hline
 \multicolumn{3}{|c|}{otazky} \\
 \hline
 PK& id & \\
 & tema    &text\\
 &otazka  &text   \\
 &spravna\_odpoved &text\\
 &spatna\_odpoved1 &text\\
 &spatna\_odpoved2&text  \\
 &spatna\_odpoved3&text   \\
 \hline
\end{tabular}
\end{center}
\end{table}
...
Metoda \texttt{otazka} v~databázi vyhledá otázku se zadaným číslem a vrátí ji jako list. 
Funkce \texttt{pridejOtazku} přidává do databáze otázky.
...
První metodou třídy \texttt{App} je \texttt{on\_execute}, která obsluhuje hlavní smyčku aplikace. Pokud je proměnná \texttt{\_running} rovna \texttt{True}, tzn. pokud má aplikace běžet, nastaví tato metoda pozadí na bílou barvu a zavolá metody \texttt{on\_event} a \texttt{on\_render}. V~opačném případě aplikaci zavře.
...
Tento modul má třídu \texttt{Button}. Tato třída umožňuje vytvoření tlačítka a práci s~ním. Má čtyři parametry: \texttt{x}, \texttt{y}, \texttt{obrazek} a \texttt{text}. Parametry \texttt{x} a \texttt{y} udávají souřadnice levého horního rohu tlačítka. Parametr \texttt{obrazek}, který slouží pro určení obrázku, který se použije pro vykreslení tlačítka, a parametr \texttt{text} udává text, který bude na tlačítku vykreslen. Tento parametr není povinný a jeho výchozí hodnota je prázdný řetězec. Tato třída má dvě metody. První, s~názvem \texttt{handle\_event}, slouží pro zjišťování, zda hráč na tlačítko klikl. Druhá metoda s~názvem \texttt{zobraz} vykreslí tlačítko na zadanou plochu a poté na toto tlačítko vypíše zadaný text.
 \begin{lstlisting}[language=Python, caption=Metoda \texttt{handle\_event} třídy \texttt{Button}, extendedchars=true , literate={á}{{\'a}}1 {ý}{{\'y}}1 {ě}{{\v{e}}}1 {š}{{\v{s}}}1 {č}{{\v{c}}}1 {ž}{{\v{z}}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {é}{{\'e}}1 {ď}{{\v{d}}}1 {ň}{{\v{n}}}1 {ť}{{\v{t}}}1 {ů}{{\r{u}}}1 {ú}{{\'u}}1 {ř}{{\v{r}}}1 {Á}{{\'A}}1 {Ý}{{\'Y}}1 {Ě}{{\v{E}}}1 {Š}{{\v{S}}}1 {Č}{{\v{C}}}1 {Ž}{{\v{Z}}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {É}{{\'E}}1 {Ď}{{\v{D}}}1 {Ň}{{\v{N}}}1 {Ť}{{\v{T}}}1 {Ů}{{\r{U}}}1 {Ú}{{\'U}}1 {Ř}{{\v{R}}}1]
...
Poté jsem přidala hlavní menu. To mělo zatím jen dvě tlačítka: „Hrát“ a „Opustit“. Tato tlačítka byla opět vytvořena pomocí modulu \texttt{button}. Tlačítko „Hrát“ zobrazilo hráči otázku a tlačítko „Opustit“ zavřelo aplikaci.\\

\section{Verze 2.1 Přidání souboje}

Jako další jsem přidala třídu \texttt{Tvor} a třídu \texttt{Hrac}. Tyto třídy nemají žádné metody a slouží tedy jen pro vytváření instancí tvorů a hráče s~různými parametry. Také jsem vytvořila list se všemi druhy tvorů v~této hře a přidala ho do proměnné \texttt{tvori}.

 \begin{lstlisting}[language=Python, caption=Proměnná \texttt{tvori}, extendedchars=true , literate={á}{{\'a}}1 {ý}{{\'y}}1 {ě}{{\v{e}}}1 {š}{{\v{s}}}1 {č}{{\v{c}}}1 {ž}{{\v{z}}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {é}{{\'e}}1 {ď}{{\v{d}}}1 {ň}{{\v{n}}}1 {ť}{{\v{t}}}1 {ů}{{\r{u}}}1 {ú}{{\'u}}1 {ř}{{\v{r}}}1 {Á}{{\'A}}1 {Ý}{{\'Y}}1 {Ě}{{\v{E}}}1 {Š}{{\v{S}}}1 {Č}{{\v{C}}}1 {Ž}{{\v{Z}}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {É}{{\'E}}1 {Ď}{{\v{D}}}1 {Ň}{{\v{N}}}1 {Ť}{{\v{T}}}1 {Ů}{{\r{U}}}1 {Ú}{{\'U}}1 {Ř}{{\v{R}}}1]
...
\begin{lstlisting}[language=Python, caption=Metoda \texttt{souboj}]
...
\section{Verze 2.2 Přidání mapy} 
 
Jako další jsem přidala mapu. Mapa byla původně tvořena čtvercovou sítí o~velikosti 20×20, z~čehož mohl hráč vidět při pohybu po mapě 7×7 okolí kolem něho. Nakonec jsem se ale rozhodla použít síť pouze 10×10 a hráči zobrazovat pouze oblast 3×3. Pohyb po mapě je tak jasnější, obzvlášť díky větším políčkům. Mapa je definována v~proměnné mapa třídy \texttt{App} jako slovník, ve kterém je ke každé souřadnici přiřazen typ políčka.

\begin{lstlisting}[language=Python, caption=Proměnná \texttt{mapa}]
...
\section{Verze 2.3 Přidání obchodu}

V~této malé verzi jsem přidala obchod a třídu \texttt{Predmet}. Tato třída má tři parametry a~těmi jsou \texttt{nazev}, \texttt{obrazek}, \texttt{cena}. Tato třída má jednu metodu \texttt{pouzit}. Tato metoda buď přidá hráči pět poškození, pokud se předmět jmenuje \textit{Lektvar síly}, anebo hráče vyléčí a přidá 20~životů, pokud se jmenuje \textit{Lektvar zdraví}.

\begin{lstlisting}[language=Python, caption=Metoda \texttt{pouzit}, extendedchars=true , literate={á}{{\'a}}1 {ý}{{\'y}}1 {ě}{{\v{e}}}1 {š}{{\v{s}}}1 {č}{{\v{c}}}1 {ž}{{\v{z}}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {é}{{\'e}}1 {ď}{{\v{d}}}1 {ň}{{\v{n}}}1 {ť}{{\v{t}}}1 {ů}{{\r{u}}}1 {ú}{{\'u}}1 {ř}{{\v{r}}}1 {Á}{{\'A}}1 {Ý}{{\'Y}}1 {Ě}{{\v{E}}}1 {Š}{{\v{S}}}1 {Č}{{\v{C}}}1 {Ž}{{\v{Z}}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {É}{{\'E}}1 {Ď}{{\v{D}}}1 {Ň}{{\v{N}}}1 {Ť}{{\v{T}}}1 {Ů}{{\r{U}}}1 {Ú}{{\'U}}1 {Ř}{{\v{R}}}1]
def pouzit(self):
...
Když je hráč v~obchodě, vidí dvě tlačítka, každé slouží k~nákupu jednoho z~lektvarů. Dále se hráči zobrazuje, kolik zlaťáků mu zbývá. V~levém horním rohu obrazovky se nachází tlačítko „Zpět“, díky kterému se může hráč dostat zpět na mapu.

\section{Verze 2.4 Přidání možnosti vložit otázku}

Poté jsem přidala možnost přidat otázku do databáze. Aby mohl uživatel zadat všechny části otázky, bylo nutné vytvořit třídu pro práci s~textovými poli. Tuto třídu jsem přidala do modulu \texttt{button} a pojmenovala \texttt{Textbox}. Na rozdíl od třídy \texttt{Button} má pouze tři parametry. Chybí jí parametr \texttt{text}, ostatní jsou stejné. Má také metody \texttt{handle\_event} a \texttt{zobraz}. Metoda zobraz je stejná jako u~třídy \texttt{Button}, metoda \texttt{handle\_event} se ale liší tím, že zjišťuje, zda má být textové pole aktivní, a pokud ano, ukládá, co uživatel píše, do proměnné \texttt{text}. Textové pole je aktivní, pokud na něj uživatel klikne. Jakmile ale klikne mimo něj, přestává být aktivní.

\begin{lstlisting}[language=Python, caption=Metoda \texttt{handle\_event} třídy \texttt{Textbox}, extendedchars=true , literate={á}{{\'a}}1 {ý}{{\'y}}1 {ě}{{\v{e}}}1 {š}{{\v{s}}}1 {č}{{\v{c}}}1 {ž}{{\v{z}}}1 {í}{{\'i}}1 {ó}{{\'o}}1 {é}{{\'e}}1 {ď}{{\v{d}}}1 {ň}{{\v{n}}}1 {ť}{{\v{t}}}1 {ů}{{\r{u}}}1 {ú}{{\'u}}1 {ř}{{\v{r}}}1 {Á}{{\'A}}1 {Ý}{{\'Y}}1 {Ě}{{\v{E}}}1 {Š}{{\v{S}}}1 {Č}{{\v{C}}}1 {Ž}{{\v{Z}}}1 {Í}{{\'I}}1 {Ó}{{\'O}}1 {É}{{\'E}}1 {Ď}{{\v{D}}}1 {Ň}{{\v{N}}}1 {Ť}{{\v{T}}}1 {Ů}{{\r{U}}}1 {Ú}{{\'U}}1 {Ř}{{\v{R}}}1]
def handle_event(self, event):
    if event.type == pygame.MOUSEBUTTONUP:
        if self.rect.collidepoint(event.pos):
            self.aktivni = True
...
\end{lstlisting}
  ...
  Na hlavní menu jsem tedy přidala tlačítko „Přidat otázku“, na které když hráč klikne, zobrazí se mu rozhraní pro přidání otázky. Toto rozhraní obsahuje šest textových polí vytvořených pomocí třídy \texttt{Textbox}, každé pro jednu část otázky. Dále je na obrazovce tlačítko „Zpět“, které vrátí hráče na hlavní menu a tlačítko „Přidat otázku“, které přidá zadanou otázku do databáze.

\section{Verze 2.5 Rozdělení otázek tvorům podle témat}

Nakonec jsem se rozhodla rozdělit otázky jednotlivým druhům tvorů podle tématu. Takto může hráč lépe předpokládat obor, ze kterého otázka bude a může si tak některá témata cíleněji procvičit. Abych toto mohla udělat, musela jsem nejdříve vytvořit další dvě metody třídy \texttt{Db\_otazek} v~modulu \texttt{databaze}. Jedna z~těchto metod se jmenuje \texttt{vsechna\_temata} a~vrací všechna témata otázek, která se v~databázi vyskytují. Druhá metoda se jmenuje \texttt{tema} a vrací list čísel všech otázek, které mají určité téma. Poté jsem přidala parametr \texttt{otazky} třídě \texttt{Tvor}, jehož výchozí hodnota je prázdný list. V~konstruktoru třídy \texttt{App} se k~tomuto listu připojí listy otázek jednotlivých témat, která budou k~tomuto tvoru přiřazena. Témata se tvorům přiřazují automaticky a to podle zbytku po celočíselném dělení pořadí tématu počtem všech tvorů.

\begin{lstlisting}[language=Python, caption=Přiřazení otázek tvorům]
...
\begin{figure}[h]
    \begin{center}
    \includegraphics[height=390pt]{schema}
...
Pokud v~nějakém souboji hráč zemře, zobrazí se mu na obrazovce zpráva „Zemřel jsi“, celkové dosažené skóre a dvě tlačítka. Pomocí jednoho může hráč hru ukončit, pomocí druhého hrát znovu. Hráči se vynuluje skóre a zlaťáky a hráči i tvorům se vrátí poškození a životy na počáteční hodnoty.

\begin{figure}[h]
    \centering
    \begin{minipage}{.5\textwidth}
        \centering
        \fbox{\includegraphics[height=115pt]{game_over}}
        \caption{Konec hry}
        \label{gameover}
    \end{minipage}
\end{figure}
...
Dále práce představila pojem \textit{agilní metoda} a některé konkrétní takové metody. Při analýze dostupných zdrojů bylo zjištěno, že nelze určit, zda jsou tyto agilní metody efektivnější než klasické postupy. Podle těchto zdrojů jsou však názory vývojářů i zákazníků na tyto metody lepší než na klasické postupy.
...
\begin{appendices}
	...
	\begin{enumerate}
		\item{\texttt{button.py} — modul sloužící pro práci s~tlačítky a textovými poly}
		\item{\texttt{databaze.py} — modul sloužící pro komunikaci s~databází}
		\item{\texttt{main.py} — vykreslování a logika hry}
		\item{\texttt{otazky.db} — databáze otázek}
		\item{\texttt{otazky.py} — modul sloužící pro tvorbu otázek}
		\item{složka \texttt{obrazky} — Obsahuje obrázky využívané ve hře}
	\end{enumerate}
	Aplikaci je možné spustit v~příkazovém řádku zadáním příkazů:
	
	
	 \texttt{cd cesta\textbackslash do\textbackslash složky\textbackslash obsahující\textbackslash program}
