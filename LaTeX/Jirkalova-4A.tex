...
\chapter*{Úvod}     % úvod práce 
	...
	V hlavní části práce se pokusím vyvinout svou vlastní vědomostní hru. Budu se přitom snažit co nejvíce zásadami agilního vývoje. Pro psaní kódu využiji programovací jazyk \texttt{Python} a jeho rozšíření \texttt{Pygame}. V práci poté popíši postup vývoje této hry a její konečnou podobu.
...
Další metodou třídy \texttt{Otazka} je \texttt{tah}. Má jeden parametr \texttt{hrac} (objekt třídy \texttt{Hrac}). Tato metoda vybere náhodnou otázku pomocí funkce \texttt{randint} modulu \texttt{random}. Poté zavolá metodu zeptej a pokud vrátí \texttt{True}, připíše hráči jeden bod.
...
\begin{figure}[h]
    ...
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[height=115pt]{pridej_otazku}
...
\begin{figure}[h]
    ...
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[height=115pt]{obchod}
        \caption{Obchod}
        \label{obchod}
    \end{minipage}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{minipage}{.5\textwidth}
        \centering
        \includegraphics[height=115pt]{game_over}
        \caption{Konec hry}
        \label{gameover}
    \end{minipage}
\end{figure}
